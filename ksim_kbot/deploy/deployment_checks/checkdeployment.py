...
def plot_command_data(data, output_dir="plots"):
    ...
    pos_figs = [(fig_pos_1, axs_pos_1), (fig_pos_2, axs_pos_2), 
                (fig_pos_3, axs_pos_3), (fig_pos_4, axs_pos_4)]
    vel_figs = [(fig_vel_1, axs_vel_1), (fig_vel_2, axs_vel_2), 
                (fig_vel_3, axs_vel_3), (fig_vel_4, axs_vel_4)]
...
def plot_vector_data(data, key, output_dir="plots"):
    ...
    if key == "pos_diff" or key == "vel_obs" or key == "prev_action":
        ...
        magnitudes = np.sqrt(np.array(accel_x)**2 + np.array(accel_y)**2 + np.array(accel_z)**2 +
                             np.array(gyro_x)**2 + np.array(gyro_y)**2 + np.array(gyro_z)**2)
...
if __name__ == "__main__":
    ...
    if deployment_data:
        ...
        print(f"Successfully loaded deployment data with keys: {list(data.keys()) if isinstance(data, dict) else 'not a dictionary'}")
