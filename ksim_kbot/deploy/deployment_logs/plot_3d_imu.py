...
def plot_3d_imu(data: dict, output_dir: str, hz: int, skip_factor: int) -> None:
    ...
    logger.info(f"Using {frame_count} quaternion frames (skipping every {skip_factor} frames)")
    
    # Log first frame Euler angles
    if frame_count > 0:
        w, x, y, z = quat_data[0]
        euler = quaternion_to_euler(w, x, y, z)
        logger.info(f"Initial orientation (Euler angles xyz): Roll={euler[0]:.2f}°, Pitch={euler[1]:.2f}°, Yaw={euler[2]:.2f}°")
    ...
    def update(frame):
        ...
        if frame % 10 == 0:
            euler = quaternion_to_euler(w, x, y, z)
            logger.info(f"Frame {frame} orientation: Roll={euler[0]:.2f}°, Pitch={euler[1]:.2f}°, Yaw={euler[2]:.2f}°")
    ...
    logger.info(f"Creating animation with {frame_count} frames at {hz} Hz")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Frequency (frames per second) for the output video (default: 30)"
    )
    parser.add_argument(
        "--skip",
        type=int,
        default=2,
        help="Skip factor for frames (e.g., 2 uses every other frame, default: 1)"
    )
